...
async function updateUser({
  ...
  token: string | null;
  id: string;
  data: zod.infer<typeof UpdateUserFormSchema>;
}) {
  ...
  const response = await axios.put(routes.api.admin.users.url(id), formData, {
    headers: {
      Authorization: `Bearer ${token}`,
...
export function EditUser({
  ...
  const form = useForm<zod.infer<typeof UpdateUserFormSchema>>({
  ...
  useEffect(() => {
    ...
    form.setValue("sku", user.sku);
    form.setValue("stock", user.stock);
    form.setValue("price", user.price);
    form.setValue("salePrice", user.salePrice ?? 0);
    form.setValue("categoryId", user.category.id);

    const imageUrls = user.pictureIds.map(
      (pictureId) => `${process.env.NEXT_PUBLIC_FILE_URL}/${pictureId}`,
    );

    setUserImages(imageUrls.map((url) => ({ url, isNew: false })));
  }, [form.setValue, user]);
  ...
  const handleImageUpload = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      const availableSlots = 5 - userImages.length;
  ...
  const handleImageRemove = (indexToRemove: number) => {
    const updatedUserImages = userImages.filter((image, index) => {
      if (index === indexToRemove) {
        if (!image.isNew) {
          const picturesToBeDeleted = form.getValues("pictureIds");

          const pictureIdToRemove = user.pictureIds.find(
            (pictureId) =>
          ...
          const updatedPicturesToBeUploaded =
            currentPicturesToBeUploaded.filter((_, i) => {
              const newImagesBeforeIndex = userImages
              ...
              return (
                i !==
                indexToRemove -
                  (userImages.length - currentPicturesToBeUploaded.length) -
  ...
  const onSubmit = (data: zod.infer<typeof UpdateUserFormSchema>) => {
    updateUserMutation.mutate({ token, id: user.id, data });
  };
    ...
    <Dialog open={isEditUserOpen} onOpenChange={setIsEditUserOpen}>
      ...
      <DialogContent className="sm:max-w-[768px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the details of your existing user.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
              ...
              <div className="flex-[2] flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-lg p-4 h-64">
                ...
                <div className="relative group cursor-pointer mt-4">
                  <Button type="button" variant="outline">
                    ...
                    {userImages.length > 0 ? "Change Images" : "Upload Images"}
                  </Button>
                  <Input
