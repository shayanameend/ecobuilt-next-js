...
async function createUser({
  token,
  data,
}: { token: string | null; data: zod.infer<typeof CreateUserFormSchema> }) {
...
export function NewUser() {
  ...
  const [productImages, setUserImages] = useState<string[]>([]);

  const form = useForm<zod.infer<typeof CreateUserFormSchema>>({
      ...
      for (const file of limitedFileArray) {
        ...
        reader.onload = () => {
          ...
          if (newImagePreviews.length === totalFiles) {
            setUserImages(newImagePreviews);
          }
        };
  ...
  const handleImageRemove = (indexToRemove: number) => {
    ...
    setUserImages(productImages.filter((_, index) => index !== indexToRemove));
  };
  ...
  const onSubmit = (data: zod.infer<typeof CreateUserFormSchema>) => {
    createUserMutation.mutate({ token, data });
  };
